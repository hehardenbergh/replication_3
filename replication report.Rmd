...
# authors Avidit Acharya, Matthew Blackwell, Maya Sen. I replicate Tables 1, 3, and all figures.
...
# source more code from other part of the project with this line of code

source("dataverse_files_3/panel-utils.R")
...
# The authors want to use 1860 census data to find the population of enslaved
...
cnty.iv <- ivreg(update(base.iv.form, dem ~ .), 
                 data = south.counties, 
                 weights = sample.size)
summary(cnty.iv)

cnty.aff.iv <- ivreg(update(base.iv.form, affirm  ~ .), 
                     data = south.counties, 
                     weights = sample.size)
summary(cnty.aff.iv)

cnty.resent.iv <- ivreg(update(base.iv.form, resent  ~ .), 
                        data = south.counties, 
                        weights = sample.size.res)
summary(cnty.resent.iv)

# my replication won't recognize abs.sample for some reason. figuring this out.

therm.cty.iv <- ivreg(update(base.iv.form, therm.diff ~ .), 
                      data = nes.counties, 
                      weights = sample.size.td, 
                      subset = abs.sample == 1)
summary(therm.cty.iv)

cnty.res.first <- lm(base.first.form, 
                     data = south.counties)
...
cnty.dem.rform <- lm(update(rform.form, dem ~ .), 
                     data = south.counties, 
                     weights = sample.size)
summary(cnty.dem.rform)

cnty.dem.ns.rform <- lm(update(rform.form, dem ~ .), 
                        data = wh.counties, 
                        weights = sample.size,
                        subset = !(state.abb %in% st.list) & !(state.abb %in% c("MD", "DE","KY", "MO", "WV")))
summary(cnty.dem.ns.rform)

cnty.aff.rform <- lm(update(rform.form, affirm ~ .),
                     data = south.counties, 
                     weights = sample.size)
summary(cnty.aff.rform)

cnty.aff.ns.rform <- lm(update(rform.form, affirm ~ .), 
                        data = wh.counties, 
                        weights = sample.size, 
                        subset = !(state.abb %in% st.list) & !(state.abb %in% c("MD", "DE","KY", "MO", "WV")))
...
cnty.therm.rform <- lm(update(rform.form, therm.diff ~ .), 
                       data = nes.counties, 
                       weights = sample.size.td, 
                       subset = abs.sample == 1)
...
# My code has a problem running this line 
